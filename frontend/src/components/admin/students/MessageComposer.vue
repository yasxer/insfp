    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
          {{ recipientCount === 0 ? 'Broadcast Message' : `Send Message to ${recipientCount} Student${recipientCount > 1 ? 's' : ''}` }}
        </h3>
...
const recipientCount = computed(() => props.selectedStudents.length)

const recipientNames = computed(() => {
  if (props.selectedStudents.length === 0) {
    return 'All students in the current view'
  }
  if (props.selectedStudents.length <= 3) {
    return props.selectedStudents.map(s => s.full_name).join(', ')
  }
  return `${props.selectedStudents.slice(0, 2).map(s => s.full_name).join(', ')} and ${props.selectedStudents.length - 2} others`
})
...
const handleSubmit = async () => {
  ...
  try {
    await emit('send', {
      ...
      recipients: props.selectedStudents.map(s => s.id)
    })
