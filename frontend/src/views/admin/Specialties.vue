<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h1 class="text-2xl font-semibold text-gray-900">Specialties Management</h1>
    </div>
    ...
    <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div @click="openCreateModal" class="col-span-1 bg-white rounded-lg shadow border-2 border-dashed border-gray-300 p-6 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500 hover:bg-gray-50 transition-colors min-h-[200px]">
        ...
        <h3 class="text-lg font-medium text-gray-900">Add New Specialty</h3>
      </div>

      <!-- Specialty Cards -->
      <div v-for="specialty in specialties" :key="specialty.id" @click="goToDetails(specialty.id)" class="bg-white overflow-hidden shadow rounded-lg cursor-pointer hover:shadow-md transition-shadow relative group">
        <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
           <!-- Edit Icon -->
           <button @click.stop="openEditModal(specialty)" class="text-gray-400 hover:text-gray-500">
        ...
        <div class="px-4 py-5 sm:p-6">
          <div class="flex items-center">
            ...
            <div v-else class="flex-shrink-0 h-16 w-16 rounded-md overflow-hidden bg-gray-100 border border-gray-200">
          ...
          <div class="mt-4">
            <span :class="[specialty.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800', 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize']">
